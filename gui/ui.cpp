    ...
    update_query(this)
{
    ...
    profile_menu.addAction("Create new empty config", this, [&]() -> void { (void) MainWindow::make_empty_config_internal(); });
    profile_menu.addAction("Create new copied config", this, &MainWindow::make_copied_config);
    profile_menu.addAction("Create new config from wizard", this, &MainWindow::make_config_from_wizard);
    profile_menu.addAction("Open configuration directory", this, &MainWindow::open_config_directory);
...
bool MainWindow::make_empty_config_internal()
{
    QString name;
...
void MainWindow::make_config_from_wizard()
{
    if (make_empty_config_internal())
    {
        Wizard w;
        w.exec();
    }
...
{
    if (mk_window(&options_widget, [&](bool flag) -> void { set_keys_enabled(!flag); }, *this))
    {
